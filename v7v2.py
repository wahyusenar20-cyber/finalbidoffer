...
# ==========================================
# KONFIGURASI HALAMAN & COOKIE
# ==========================================
st.set_page_config(page_title="STOCKHOLDER - MULTI RADAR", layout="wide")

# Inisialisasi Cookie Controller untuk Auto-Save
controller = CookieController()

# ==========================================
# CSS UI/UX MODERN & ULTRA COMPACT
# ==========================================
st.markdown("""
    <style>
        ...
        .stApp [data-testid="stVerticalBlock"] { opacity: 1 !important; transition: none !important; }
        .block-container { padding: 1.5rem 0.5rem 1rem 0.5rem !important; max-width: 100%; }

        .modern-table-wrapper {
            overflow-x: auto; border-radius: 6px; border: 1px solid #333333;
            background-color: #16161a; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);
            width: fit-content; max-width: 100%;
        }
        
        table.dataframe {
            width: max-content; border-collapse: collapse; text-align: center; 
            font-size: 0.85rem; font-family: 'JetBrains Mono', monospace; margin-bottom: 0 !important;
        }
        table.dataframe th {
            background-color: #1f2023; color: #a0a0a0; font-weight: 600;
            padding: 6px 6px; border-bottom: 1px solid #333333; text-transform: uppercase;
            letter-spacing: 0.5px; white-space: nowrap; text-align: center !important;
        }
        table.dataframe td {
            padding: 4px 6px; border-bottom: 1px solid #252529; color: #d4d4d4;
            vertical-align: middle; white-space: nowrap; text-align: center !important;
        }
        table.dataframe th:nth-child(2), table.dataframe td:nth-child(2) { text-align: left !important; }
        table.dataframe tr:hover td { background-color: #2b2d31; }
...
# ==========================================
# 1. INPUT TOKEN (DENGAN AUTO-SAVE COOKIE)
# ==========================================
st.sidebar.title("🔑 Akses Mesin")

# Tarik token dari memori browser jika ada
saved_token = controller.get('sb_token')
default_token = saved_token if saved_token else ""

TOKEN_INPUT = st.sidebar.text_input("Paste Token Bearer Stockbit:", value=default_token, type="password")

# Simpan otomatis jika ada token baru
if TOKEN_INPUT and TOKEN_INPUT != saved_token:
    controller.set('sb_token', TOKEN_INPUT)
...
def render_tabel_avg(data_list, rank_history):
    ...
    for i, item in enumerate(data_list):
        ...
        price_cell = f"<span style='color:#777777; font-size:0.75em;'>{item['best_bid']:,}-{item['best_off']:,}</span>"
        ...
        rows.append({
            ...
            "Saham": ticker_cell, "Lot": l_str, "Freq": f_str, "Harga": price_cell, 
...
# ==========================================
# 5. UI UTAMA & NAVIGASI (DENGAN AUTO-RECONNECT)
# ==========================================
st.sidebar.markdown("---")
st.sidebar.title("📊 RADAR MULTI-MODE")

# Tarik pilihan Mode terakhir
last_mode = controller.get('sb_mode') or "📁 Mode CSV (Auto Filter)"
mode_index = 0 if last_mode == "📁 Mode CSV (Auto Filter)" else 1
mode = st.sidebar.radio("Pilih Sumber Data:", ["📁 Mode CSV (Auto Filter)", "📝 Mode TXT (Custom Watchlist)"], index=mode_index)

if mode != last_mode:
    controller.set('sb_mode', mode)

st.sidebar.markdown("---")

# Tarik settingan terakhir
jeda_auto = st.sidebar.number_input("Interval Refresh (Detik):", 1, 3600, int(controller.get('sb_jeda') or 6))
kecepatan_turbo = st.sidebar.number_input("Kecepatan Scan (1-10x):", 1, 10, int(controller.get('sb_turbo') or 10))

if str(jeda_auto) != str(controller.get('sb_jeda')): controller.set('sb_jeda', jeda_auto)
if str(kecepatan_turbo) != str(controller.get('sb_turbo')): controller.set('sb_turbo', kecepatan_turbo)
...
# Pengecekan status terakhir apakah sedang jalan sebelum disconnect
is_running = controller.get('sb_is_running') == "True"

if mode == "📁 Mode CSV (Auto Filter)":
    st.sidebar.markdown("**📁 Sumber Data:** `Ringkasan_Saham.csv` *(Auto-Read)*")
    min_freq = st.sidebar.number_input("Min Freq:", 0, value=int(controller.get('sb_min_freq') or 1000))
    if str(min_freq) != str(controller.get('sb_min_freq')): controller.set('sb_min_freq', min_freq)
    
    limit_scan = st.sidebar.selectbox("Limit Scan:", [20, 50, 100, 200, 300, 400, 500, 600], index=6)
    
    # TRIGGER AUTO-RECONNECT: Jika tombol ditekan ATAU state browser menandakan aplikasi sedang running
    if st.sidebar.button("🚀 START SCAN CSV") or (is_running and 'active_tickers' not in st.session_state):
        try:
            ...
            controller.set('sb_is_running', "True") # Simpan state nyala ke Cookie
        except FileNotFoundError:
            st.sidebar.error("⚠️ File 'Ringkasan_Saham.csv' tidak ditemukan di server!")
            controller.set('sb_is_running', "False")

else:
    file_txt = st.sidebar.file_uploader("Upload Watchlist.txt", type=["txt"])
    # Note: Auto-reconnect pada upload TXT agak tricky karena isi file tidak tersimpan di cookie.
    # Disarankan merubah ke mode CSV jika ingin 100% full autonomous saat HP ditinggal.
    if st.sidebar.button("🎯 START WATCHLIST TXT"):
        if file_txt:
            ...
            st.session_state['radar_active'] = True
            controller.set('sb_is_running', "True")

if st.session_state.get('radar_active'):
    if st.sidebar.button("🛑 STOP RADAR"): 
        controller.set('sb_is_running', "False") # Matikan state di Cookie
